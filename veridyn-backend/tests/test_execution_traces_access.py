...
def test_get_execution_traces_own_test_run(client, authenticated_context):
    """
    Test A — Own TestRun:
    Call GET /test-runs/{YOUR_TEST_RUN_ID}/traces
    Expected: 200 OK
    """
    test_run_id = authenticated_context["own_test_run_id"]
    headers = authenticated_context["headers"]

    response = client.get(f"/test-runs/{test_run_id}/traces", headers=headers)
...
def test_get_execution_traces_unknown_test_run(client, authenticated_context):
    """
    Test B — Unknown TestRun:
    Call GET /test-runs/00000000-0000-0000-0000-000000000001/traces
    Expected: 404 Not Found with detail 'Test run not found'
    """
    unknown_id = "00000000-0000-0000-0000-000000000001"
    headers = authenticated_context["headers"]

    response = client.get(f"/test-runs/{unknown_id}/traces", headers=headers)
...
def test_create_execution_trace_unauthorized(client, authenticated_context):
    """
    Test C — Unauthorized Trace creation:
    Call POST /test-runs/00000000-0000-0000-0000-000000000001/traces
...
def test_get_execution_traces_missing_jwt(client, authenticated_context):
    """
    ...
    response = client.get(f"/test-runs/{test_run_id}/traces")
